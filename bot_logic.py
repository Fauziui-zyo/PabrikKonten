...
# Inisialisasi Gemini dengan SDK baru (google.genai)
def initialize_gemini():
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Test connection menggunakan model terbaru
...
def generate_health_content(max_retries: int = 3) -> Dict:
    """Menggunakan Gemini untuk mencari ide kesehatan"""
    prompt = """
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Syntax baru untuk generate content
...
def generate_image_hf(image_prompt: str, max_retries: int = 3) -> Optional[str]:
    """Meminta model AI FLUX di Hugging Face"""
...
def main():
    try:
        ...
        if not image_result:
            logger.error("❌ Gambar gagal dibuat. Menghentikan proses.")
            return False
